...
class PageTwoViewMore extends StatefulWidget {
  const PageTwoViewMore({Key? key}) : super(key: key);

  @override
  State<PageTwoViewMore> createState() => _PageTwoViewMoreState();
}

class _PageTwoViewMoreState extends State<PageTwoViewMore> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: const PageTwoViewMoreScreen(),
    );
  }
}

class PageTwoViewMoreScreen extends StatelessWidget {
  const PageTwoViewMoreScreen({Key? key}) : super(key: key);
      ...
      slivers: [
        SliverToBoxAdapter(
          child: Container(height: 350, child: InkWell(
            onTap: () {
              Route route =
              MaterialPageRoute(builder: (c) => const PageTwoProducts());
              Navigator.push(context, route);
            },
            child: FutureBuilder(
                future: _getImage(context, "/sliderimages/two/viewimagetwo.jpg"),
        ...
        StreamBuilder<QuerySnapshot>(
          stream: FirebaseFirestore.instance
              .collection("items")
              .where("price", isLessThanOrEqualTo: 199)
              .limit(1)
              .snapshots(),
