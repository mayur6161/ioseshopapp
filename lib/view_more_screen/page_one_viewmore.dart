...
class PageOneViewMore extends StatefulWidget {
  const PageOneViewMore({Key? key}) : super(key: key);

  @override
  State<PageOneViewMore> createState() => _PageOneViewMoreState();
}

class _PageOneViewMoreState extends State<PageOneViewMore> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: const PageOneViewMoreScreen(),
    );
  }
}

class PageOneViewMoreScreen extends StatelessWidget {
  const PageOneViewMoreScreen({Key? key}) : super(key: key);
      ...
      slivers: [
        SliverToBoxAdapter(
          child: Container(height: 350, child: InkWell(
            onTap: () {
              Route route =
              MaterialPageRoute(builder: (c) => const PageOneProducts());
              Navigator.push(context, route);
            },
            child: FutureBuilder(
                future: _getImage(context, "/sliderimages/one/viewimageone.jpg"),
        ...
        StreamBuilder<QuerySnapshot>(
          stream: FirebaseFirestore.instance
              .collection("items")
              .where("price", isGreaterThanOrEqualTo: 199)
              .limit(1)
              .snapshots(),
